...
const revengeOfBadger = {
  ...
  // updateClapCount: function(name){
  //   if(name in this.clapCount){
  //     this.clapCount[name]++;
  //     console.log(`${name}'s new clap count: ${this.clapCount[name]}`);
  //   } else {
  //     console.log(`Sorry, that student doesn't exist.`);
  //   }
  // },

  // ONLY LOOPING THROUGH HANNAH???
  getFridayDrawList: function(){
    this.fridayDrawList = [];

    for(let key in this.clapCount){
      console.log(key);
      if(this.clapCount[key] >= 2){
        this.fridayDrawList.push(key);
      }
    }
      return this.fridayDrawList.length;
  },

  // printFridayDrawList: function(){
  //   console.log('*** Friday Draw List ***');
  //   const list = this.getFridayDrawList();
  //   console.log(list);
  // },

  // pickRandomStudent: function(){
  //   const random = Math.random() * fridayDrawList.length;
  //   const randomNumner = Math.floor(random);
  //   console.log(randomNumber);
  //   const randomStudent = fridayDrawList[randomNumber];
  //   console.log(`This week's Friday warm solution will be delivered by: ${randomStudent}`);
