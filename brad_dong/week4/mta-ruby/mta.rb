...
def one_line (line, start, finish)
  start_index = $mta[line].index(start)
  finish_index = $mta[line].index(finish)
  # puts line
  # puts line.class
  stations = $mta[line][finish_index...start_index].join(', ')
  puts "You need to travel on the #{line} line, from #{start} to #{finish}, through #{start_index-finish_index} stations.  You pass through #{stations} stations."
end




one_line(chosen_line,origin,destination)
# one_line(:n,origin,destination)
